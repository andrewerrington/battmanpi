...
class startDialog(tkSimpleDialog.Dialog):
    ...
    def body(self, master):
        ...
        self.batteryGroup.grid(column=0, row=0, columnspan=2,
            sticky=(tk.N, tk.W, tk.E, tk.S), padx=6, pady=(6,0))

        ttk.Label(self.batteryGroup, text="Cell Type:",
            underline=5).grid(column=0, row=0, sticky=tk.E, padx=(4,0), pady=4)

        self.batteryCellTypeCombo=ttk.Combobox(self.batteryGroup,
            state='readonly', width=10, values=["NiCd","NiMH","LiPo","LiNP","PbAcid"])  # FIXME: Get this list from somewhere
        ...
        ttk.Label(self.batteryGroup, text="mAh").grid(column=4, row=0,
            sticky=tk.W, padx=(4,0), pady=4)

        ttk.Label(self.batteryGroup, text="Number of Cells:", underline=0)\
            .grid(column=5, row=0, sticky=tk.E, padx=(4,0), pady=4)

        self.batteryNumberOfCells=tk.Spinbox(self.batteryGroup, width=5,
            from_=1, to=12)
        ...
        self.dischargeGroup.grid(column=0, row=1, sticky=(tk.N, tk.W, tk.E, tk.S),
            padx=(6,0), pady=(6,0))
        ...
        dischargeRateScale=ttk.Scale(self.dischargeGroup,
            command=self.updateDischargeRate, variable=self.dischargeRate,
            from_=1, to=settings.numChargeRates-1, orient=tk.HORIZONTAL)
        ...
        self.chargeGroup.grid(column=1, row=1, sticky=(tk.N, tk.W, tk.E, tk.S),
            padx=6, pady=(6,0))
        ...
        chargeRateScale=ttk.Scale(self.chargeGroup,
            command=self.updateChargeRate, variable=self.chargeRate,
            from_=1, to=settings.numChargeRates-1, orient=tk.HORIZONTAL)
        ...
        self.autoCycleGroup.grid(column=0, row=2, columnspan=2,
            sticky=(tk.N, tk.W, tk.E, tk.S), padx=6, pady=(6,0))

        ttk.Label(self.autoCycleGroup, text="Maximum Number of Cycles:",
            underline=0).grid(column=0, row=0, sticky=tk.E, padx=(4,0), pady=4)
        ...
        autoCycleStop=ttk.Checkbutton(self.autoCycleGroup,
            text="Stop when capacity increases by less than 0.5%", underline=5,
        ...
        autoCycleSave=ttk.Checkbutton(self.autoCycleGroup,
            text="Save graph as BMP file after each cycle", underline=9,
        ...
        ttk.Button(self, text="Start", underline=0, compound=tk.TOP,
            image=self.startIcon, command=self.ok).grid(column=0, row=3,
            sticky=tk.W, padx=6, pady=6)
        ttk.Button(self, text="Cancel", underline=5, compound=tk.TOP,
            image=self.cancelIcon, command=self.cancel).grid(column=1,
            row=3, sticky=tk.E, padx=6, pady=6)
    ...
    def updateDischargeRate(self, *args):
        self.dischargeRateLabel['text']="%4.0f mA"\
            %(settings.dischargeRates[self.dischargeRate.get()]*1000)


    def updateChargeRate(self, *args):
        self.chargeRateLabel['text']="%4.0f mA"\
            %(settings.chargeRates[self.chargeRate.get()]*1000)
    ...
    def validate(self):
        if self.kwargs['mode'] is not 'resistance':
            try:
                ...
                if batteryCapacity < 1 or \
                    batteryCapacity > settings.MAX_CAPACITY:
                    raise ValueError
                return 1
            except ValueError:
                messagebox.showwarning("Error",
                    "Please enter a value from 1 to %s for the Capacity"\
